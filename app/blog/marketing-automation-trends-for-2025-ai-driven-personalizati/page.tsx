...
async function getBlogPost(): Promise<BlogPost | null> {
  try {
    const filePath = path.join(process.cwd(), 'public', 'blog-data', 'marketing-automation-trends-for-2025-ai-driven-personalizati.json')
    const fileContent = fs.readFileSync(filePath, 'utf-8')
    const parsed = JSON.parse(fileContent)
    return parsed.blogPost || parsed
  } catch (error) {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: post.seo?.canonical || 'https://www.lookatmyprofile.org/blog/marketing-automation-trends-for-2025-ai-driven-personalizati'
    }
  }
}
